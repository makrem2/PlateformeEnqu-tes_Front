...
export class MyprofileComponent implements OnInit, OnDestroy {
  ...
  getOneEntreprise() {
    this.entrepriseService
      .getOneEntreprise(localStorage.getItem('Id_Key'))
      .subscribe((data) => {
  ...
  updatePassword(): void {
    ...
    this.setLoadingState(true);
    this.userService
      .updateUserPassword(this.User_id, this.passwordForm.value)
      .pipe(
        takeUntil(this.destroy$),
        finalize(() => this.setLoadingState(false))
      )
      .subscribe({
        next: (res) => {
          this.toastr.success(
            res.message,
            'You need to login again to see the changes'
          ),
            localStorage.clear();
          setTimeout(() => {
            window.location.reload();
  ...
  updateEntreprise() {
    ...
    Object.keys(this.entrepriseForm.controls).forEach((key) => {
      const value = this.entrepriseForm.get(key)?.value;
      if (value !== null) formdata.append(key, value);
    });

    this.entrepriseService
      .updateEntreprise(this.User_id, this.entrepriseForm.value)
      .pipe(
        takeUntil(this.destroy$),
        finalize(() => this.setLoadingState(false))
      )
  ...
  onImageError(event: any) {
    event.target.src =
      'https://img.freepik.com/free-psd/3d-illustration-person-with-sunglasses_23-2149436188.jpg?ga=GA1.1.1050140410.1746898981&semt=ais_hybrid&w=740';
  }
}
